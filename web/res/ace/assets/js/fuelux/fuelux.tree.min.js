	...
	c.prototype = {
														...
														if (m.type === "item") {
															...
															k
																	...
																	.html(
																			m.name);
															k.data(m);
															if ("additionalParameters" in m
																	&& "item-selected" in m.additionalParameters
																	&& m.additionalParameters["item-selected"] == true) {
																k
																...
																k
																		.find(
																				"i")
																		.removeClass(
																				f.options["unselected-icon"])
																		.addClass(
																				f.options["selected-icon"])
															}
														}
													}
		...
		selectItem : function(g) {
			...
			if (this.options.multiSelect) {
				...
				if (i[0] !== f[0]) {
					i.removeClass("tree-selected").find("i").removeClass(
			...
			if (f.hasClass("tree-selected")) {
				...
				f.find("i").removeClass(this.options["selected-icon"])
						.addClass(this.options["unselected-icon"])
			} else {
				f.addClass("tree-selected");
				f.find("i").removeClass(this.options["unselected-icon"])
		...
		selectFolder : function(g) {
			...
			var h = f.eq(0);
			var k = "."
					+ b.trim(this.options["close-icon"].replace(/\s/g, "."));
			var e, l, i;
			if (m.find(k).length) {
				e = "opened";
		...
		collapse : function() {
			...
			this.$element.find("." + this.options["open-icon"]).each(
					function() {
						var g = b(this).removeClass(this.options["open-icon"])
